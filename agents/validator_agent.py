...
from src.utils.rule_validator import run_rule_based_validation, CheckResult as RuleCheckResult
...
class ValidatorAgent:
    ...
    def validate_report(self, pdf_path: str) -> ValidationReport:
        ...
        # Run rule-based validation (deterministic checks)
        report.checks.extend(self._check_rule_based(full_text))
    ...
    def _check_rule_based(self, text: str) -> list[CheckItem]:
        """
        Run rule-based validation checks (deterministic, no LLM needed).
        These provide fast initial validation before LLM-based checks.
        """
        checks = []
        try:
            rule_results = run_rule_based_validation(text)
            for rule_result in rule_results:
                # Map rule status to check status
                status_map = {
                    "pass": "pass",
                    "fail": "fail",
                    "warning": "warning",
                ...
                checks.append(CheckItem(
                    category=f"rule_{rule_result.category}",
                    item=rule_result.rule_name,
                    status=status_map.get(rule_result.status.value, "warning"),
            ...
            checks.append(CheckItem(
                ...
                item="Rule-based validation",
                status="warning",
                details=f"Could not run rule-based checks: {e}",
                severity="medium",
            ))
        return checks
