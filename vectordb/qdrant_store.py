...
# Use unique storage path to avoid lock conflicts
_LOCAL_STORAGE_PATH = os.path.abspath("./qdrant_local_storage_v2")
os.makedirs(_LOCAL_STORAGE_PATH, exist_ok=True)
...
class GeoVectorStore:
    def __init__(
        ...
        if use_local:
            ...
            if _LOCAL_CLIENT is None:
                _LOCAL_CLIENT = QdrantClient(
                    path=_LOCAL_STORAGE_PATH,
                    force_disable_check_same_thread=True,
                )
                _set_local_client(_LOCAL_CLIENT)
                logger.info(f"Using local file-based Qdrant storage: {_LOCAL_STORAGE_PATH}")
            else:
    ...
    def _search_collection(
        ...
        """Search a single collection, return list of (text, metadata, score)."""
        try:
    ...
    def search(
        ...
        # Search main + section + rule collections (dual-index aware)
        ...
        all_results = []
        per_collection = max(top_k, top_k * 2)  # fetch extra per collection, then merge
    ...
    def _scroll_docs_from_collection(self, collection_name: str) -> list:
        """Scroll all points from a collection, returning list of (document_id, document_name, document_type)."""
        try:
    ...
    def list_documents(self) -> list[dict]:
        """List documents from main + section + rule collections (supports dual-index ingestion)."""
        docs = {}
        ...
        for coll in collections_to_scan:
            points = self._scroll_docs_from_collection(coll)
            for point in points:
    ...
    def delete_document(self, document_id: str):
        """Delete document from main + section + rule collections."""
    ...
    def get_document_chunks(self, document_id: str, limit: int = 5) -> list[dict]:
        """Get chunks from main + section + rule collections (dual-index aware)."""
    ...
    def create_section_index(self) -> str:
        """建立 Section Index for hierarchical routing"""
    ...
    def create_rule_index(self) -> str:
        """建立 Rule Index for detailed retrieval"""
    ...
    def get_section_store(self) -> 'GeoVectorStore':
        """Get or create Section Index store"""
        ...
        section_store._ensure_collection = lambda: None  # Skip re-creation check
        return section_store

    def get_rule_store(self) -> 'GeoVectorStore':
        """Get or create Rule Index store"""
        ...
        rule_store._ensure_collection = lambda: None  # Skip re-creation check
        return rule_store

    def get_by_chunk_id(self, chunk_id: str) -> Optional[Dict]:
        """Get a chunk directly by its chunk_id"""
