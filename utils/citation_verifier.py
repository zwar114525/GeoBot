...
class CitationExtractor:
    ...
    PATTERNS = [
        # [Source: document name, Section/Clause: X.X.X, Page: N] - must check first for combined format
        re.compile(
            r'\[Source:\s*([^,\]]+),\s*Section/Clause:\s*([^\],]+),\s*Page:\s*(\d+)\]',
            re.IGNORECASE
        ),
        # [Source: document name, Section/Clause: X.X.X] - without page
        re.compile(
        ...
        # [Source: document name, Section X.X.X] or [Source: document name, Clause X.X]
        re.compile(
            r'\[Source:\s*([^,\]]+),\s*(?:Section|Clause):\s*([^\]]+)\]',
            re.IGNORECASE
        ),
        # (HK CoP 2017, Clause 6.1.5) - handle both Chinese and English punctuation
        re.compile(
    ...
    def extract(self, text: str) -> List[Citation]:
        ...
        # Pattern order: full format -> short format -> parenthetical -> according to -> standalone
        
        for i, pattern in enumerate(self.PATTERNS):
            for match in pattern.finditer(text):
                # Skip if this position overlaps with already found citation
                start = match.start()
                end = match.end()
                if any(start < ep and end > sp for sp, ep in seen_positions):
                    continue
...
class CitationVerifier:
    ...
    def _documents_match(self, citation_doc: str, chunk_doc: str) -> bool:
        ...
        # Normalize for comparison - keep Unicode characters
        ...
        # Check with normalized spaces
        citation_normalized = re.sub(r'\s+', ' ', citation_norm)
        chunk_normalized = re.sub(r'\s+', ' ', chunk_norm)
        if citation_normalized in chunk_normalized or chunk_normalized in citation_normalized:
            return True
