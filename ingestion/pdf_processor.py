...
class CoPChunker:
    ...
    def _flush_text_chunk(
        ...
        if not text:
            ...
            return
        pg = current_metadata.get("page_no")
        if pg is not None and "page_number" not in current_metadata:
            current_metadata["page_number"] = pg
        if len(text) <= max_chunk_chars:
            ...
            if "page_number" not in metadata and metadata.get("page_no") is not None:
                metadata["page_number"] = metadata["page_no"]
...
def extract_text_from_pdf(pdf_path: str) -> str:
    """Extract markdown with [Page N] markers so chunk_by_sections can infer page numbers."""
        ...
        if isinstance(chunks, list) and chunks:
            ...
            for c in chunks:
                if isinstance(c, dict):
                    pn = c.get("metadata", {}).get("page_number") or (len(parts) + 1)
                    text = c.get("text", "")
                    if text:
                        parts.append(f"\n[Page {pn}]\n{text}")
            if parts:
...
def chunk_by_sections(
    ...
    def infer_page_number(text: str) -> int | None:
        """Extract page number from 'Page X' or '[Page X]' in title or text."""
        m = re.search(r"\[?Page\s+(\d+)\]?", (text or ""), re.IGNORECASE)
    ...
    for section in sections:
        ...
        if estimated_tokens <= max_chunk_tokens:
            ...
            page_no = infer_page_number(section.get("title", "")) or infer_page_number(section_text)
            doc.chunks.append(
                DocumentChunk(
                    text=section_text,
                    metadata={
